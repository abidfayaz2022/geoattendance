...
// ─────────────────────────
// Helper: resolve default center (id=1)
// ─────────────────────────
async function getDefaultCenterIdOrNull() {
  const [c] = await db.select({ id: centers.id }).from(centers).where(eq(centers.id, 1));
  return c?.id ?? null;
}
...
export function createRouter() {
  ...
  router.post("/auth/register", async (req, res) => {
    try {
      ...
      if (role === "student") {
        // keep old behavior if centerId passed, else fallback to 1 (schema also defaults)
        const fallbackCenterId = await getDefaultCenterIdOrNull();
        const effectiveCenterId = centerId ? Number(centerId) : fallbackCenterId;

        if (!effectiveCenterId) {
          return res.status(400).json({ error: "default_center_missing" });
        }

        const [center] = await db.select().from(centers).where(eq(centers.id, effectiveCenterId));
        if (!center) return res.status(400).json({ error: "center_not_found" });

        const studentInsertResult = await db.insert(students).values({
          userId: user.id,
          centerId: center.id, // can also be omitted due to default, but explicit is OK
          grade: grade || null,
  ...
  router.post("/auth/login", async (req, res) => {
    try {
      ...
      const [user] = await db
        .select()
        .from(users)
        .where(and(eq(users.email, normalizedEmail), eq(users.passwordHash, password)));
  ...
  router.get("/student/attendance-history", simpleAuth, requireRole("student"), async (req, res) => {
    try {
      const studentUserId = Number(req.user.id);

      const [student] = await db.select().from(students).where(eq(students.userId, studentUserId));
  ...
  router.get("/student/attendance/report", simpleAuth, requireRole("student"), async (req, res) => {
    try {
      const studentUserId = Number(req.user.id);
      ...
      if (!fromStr || !toStr) return res.status(400).json({ error: "from_and_to_required" });

      const fromDateRaw = parseLocalDateOnly(fromStr);
      const toDateRaw = parseLocalDateOnly(toStr);

      if (!fromDateRaw || !toDateRaw) return res.status(400).json({ error: "invalid_date_range" });
      if (fromDateRaw.getTime() > toDateRaw.getTime()) return res.status(400).json({ error: "from_after_to" });
      ...
      const buildDateListInclusive = (fromDate, toDate) => {
        const out = [];
        const cursor = new Date(fromDate.getFullYear(), fromDate.getMonth(), fromDate.getDate(), 0, 0, 0);
        const end = new Date(toDate.getFullYear(), toDate.getMonth(), toDate.getDate(), 0, 0, 0);
      ...
      for (const r of rows) {
        ...
        if (!prev) {
          byDate.set(dayKey, { firstCheckInAt: ci, lastCheckOutAt: co, sessionsCount: 1 });
        } else {
          prev.sessionsCount += 1;
          if (ci && prev.firstCheckInAt && ci.getTime() < prev.firstCheckInAt.getTime()) prev.firstCheckInAt = ci;
          if (co && (!prev.lastCheckOutAt || co.getTime() > prev.lastCheckOutAt.getTime())) prev.lastCheckOutAt = co;
        }
      }
      ...
      if (format === "json") {
        ...
        return res.json({ from: fromStr, to: toStr, calendar, sessions });
      }
      ...
      lines.push(["date", "status", "first_check_in_time_ist", "last_check_out_time_ist", "sessions"].join(","));
      ...
      for (const s of sessions) {
        lines.push([s.id, s.date || "", s.checkInTime || "", s.checkOutTime || "", s.status || ""].join(","));
      }
  ...
  // ✅ CHANGE #1: include phoneNumber + parentPhoneNumber in admin attendance report fetch
  router.get("/admin/attendance-report", simpleAuth, requireRole("admin"), async (req, res) => {
    try {
      ...
      if (fromDate && Number.isNaN(fromDate.getTime())) return res.status(400).json({ error: "invalid_dateFrom" });
      if (toDate && Number.isNaN(toDate.getTime())) return res.status(400).json({ error: "invalid_dateTo" });
      ...
      const fromStart = new Date(fromDate.getFullYear(), fromDate.getMonth(), fromDate.getDate(), 0, 0, 0);
      const toEnd = new Date(toDate.getFullYear(), toDate.getMonth(), toDate.getDate() + 1, 0, 0, 0);

      const conditions = [gte(attendanceRecords.checkInAt, fromStart), lt(attendanceRecords.checkInAt, toEnd)];
      if (centerId) conditions.push(eq(attendanceRecords.centerId, centerId));
      ...
      const rows = await db
        .select({
          ...
          distanceFromCenterCheckoutMeters: attendanceRecords.distanceFromCenterCheckoutMeters,
      ...
      const mapped = rows.map((r) => ({
        ...
        distanceFromCenterMeters: r.distanceFromCenterMeters != null ? Number(r.distanceFromCenterMeters) : null,
        distanceFromCenterCheckoutMeters:
          r.distanceFromCenterCheckoutMeters != null ? Number(r.distanceFromCenterCheckoutMeters) : null,
      }));

      res.json({
        meta: { page, pageSize, total, totalPages, dateFrom: fromStart, dateTo: toEnd },
        rows: mapped,
      });
    } catch (err) {
      console.error(err);
  ...
  router.post("/admin/attendance/scan", simpleAuth, requireRole("admin"), async (req, res) => {
    try {
      const { studentId: rawStudentId, centerId: rawCenterId, deviceId } = req.body || {};
      const studentId = Number(rawStudentId);
      const centerId = rawCenterId != null ? Number(rawCenterId) : null;

      if (!studentId || Number.isNaN(studentId)) return res.status(400).json({ error: "invalid_student_id" });
      ...
      const [{ totalSessionsToday }] = await db
        .select({ totalSessionsToday: sql`COUNT(*)`.mapWith(Number) })
        .from(attendanceRecords)
        .where(
          and(
            eq(attendanceRecords.studentId, studentId),
      ...
      if (lastTodayRecord) {
        if (openRecord && lastTodayRecord.id === openRecord.id) lastActionAt = lastTodayRecord.checkInAt;
        else lastActionAt = lastTodayRecord.checkOutAt || lastTodayRecord.checkInAt;
      }

      if (lastActionAt && now.getTime() - new Date(lastActionAt).getTime() < MIN_GAP_MS) {
      ...
      if (!openRecord && totalSessionsToday >= 2) {
        return res.status(406).json({
          ...
          message: "Maximum number of attendance sessions for today has been reached for this student.",
        });
      }
      ...
      if (openRecord) {
        ...
        const [updated] = await db.select().from(attendanceRecords).where(eq(attendanceRecords.id, openRecord.id));

        return res.json({ ok: true, action: "check_out", record: updated });
      }
      ...
      const insertedId = insertResult?.insertId ?? insertResult?.[0]?.insertId ?? null;
      ...
      return res.status(201).json({ ok: true, action: "check_in", record: newRecord });
    } catch (err) {
  ...
  router.get("/admin/stats", simpleAuth, requireRole("admin"), async (_req, res) => {
    try {
      const [studentsCountRow] = await db.select({ count: sql`count(*)` }).from(students);
      ...
      const [presentRow] = await db
        .select({ count: sql`count(*)` })
        .from(attendanceRecords)
        .where(and(eq(attendanceRecords.status, "present"), gte(attendanceRecords.checkInAt, todayStart), lt(attendanceRecords.checkInAt, tomorrowStart)));
  ...
  // ✅ CHANGE #3: /admin/students (add) include phoneNumber + parentPhoneNumber
  router.post("/admin/students", simpleAuth, requireRole("admin"), async (req, res) => {
    try {
      const { name, email, password, grade, rollNumber, phoneNumber, parentPhoneNumber } = req.body || {};

      // centerId is still required here (your existing UI sends it)
      if (!name || !email || !password) {
        return res.status(400).json({ error: "missing_fields" });
      }
      ...
      if (existingUser) return res.status(409).json({ error: "user_already_exists" });

      const userInsertResult = await db.insert(users).values({
        name,
        email: normalizedEmail,
        passwordHash: password, // TODO: hash later
        role: "student",
      });
      ...
      if (!user) {
        console.error("User not found after insert", { normalizedEmail, userInsertResult });
      ...
      const studentId = studentInsertResult?.insertId ?? studentInsertResult?.[0]?.insertId ?? null;

      const [student] = await db
        .select()
        .from(students)
        .where(studentId ? eq(students.id, studentId) : eq(students.userId, user.id));

      if (!student) {
        console.error("Student not found after insert", { userId: user.id, studentInsertResult });
        return res.status(500).json({ error: "student_creation_failed", userId: user.id });
      }
  ...
  // ✅ CHANGE #2: /admin/students/export include phone columns
  router.get("/admin/students/export", simpleAuth, requireRole("admin"), async (req, res) => {
    try {
      const centerId = req.query.centerId ? Number(req.query.centerId) : null;
      ...
      res.setHeader("Content-Type", "text/csv; charset=utf-8");
      res.setHeader("Content-Disposition", `attachment; filename="students_${centerId || "all"}.csv"`);
  ...
  router.delete("/admin/students/:studentId", simpleAuth, requireRole("admin"), async (req, res) => {
    try {
      const studentId = Number(req.params.studentId);
      if (!studentId) return res.status(400).json({ error: "invalid_student_id" });
      ...
      if (user && user.role === "student") await db.delete(users).where(eq(users.id, user.id));
  ...
  // ✅ CHANGE #4: import includes phoneNumber + parentPhoneNumber, and default centerId=1 if centerCode missing
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        if (lines.length < 2) return res.status(400).json({ error: "csv_has_no_data_rows" });
        ...
        const idxPhoneNumber = idx("phoneNumber");
        const idxParentPhoneNumber = idx("parentPhoneNumber");

        for (const col of ["name", "email", "password"]) {
          if (idx(col) === -1) {
            return res.status(400).json({ error: "missing_required_column", column: col });
          }
        }

        const defaultCenterId = await getDefaultCenterIdOrNull();
        if (!defaultCenterId) {
          return res.status(400).json({
            error: "default_center_missing",
            message: "Center with id=1 not found. Please create center id=1 or provide centerCode in CSV.",
          });
        }
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const roleRaw = idxRole !== -1 ? (parts[idxRole] || "").trim() : "";
          const grade = idxGrade !== -1 ? (parts[idxGrade] || "").trim() : "";
          const rollNumber = idxRollNumber !== -1 ? (parts[idxRollNumber] || "").trim() : "";

          // ✅ NEW
          const phoneNumber = idxPhoneNumber !== -1 ? (parts[idxPhoneNumber] || "").trim() : "";
          const parentPhoneNumber =
            idxParentPhoneNumber !== -1 ? (parts[idxParentPhoneNumber] || "").trim() : "";

          const rowNumber = i + 1;

          if (!name || !email || !password) {
            rowErrors.push({ row: rowNumber, email, error: "missing_name_email_or_password" });
            continue;
          }
          ...
          try {
            ...
            if (!user) {
              rowErrors.push({ row: rowNumber, email, error: "user_creation_failed" });
              continue;
            }
            ...
            if (role === "student") {
              // ✅ Default center: if centerCode missing, use centerId=1
              let resolvedCenterId = defaultCenterId;

              

              await db.insert(students).values({
                userId: user.id,
                centerId: resolvedCenterId, // explicit, but schema also defaults
                grade: grade || null,
                rollNumber: rollNumber || null,
                phoneNumber: phoneNumber || null,
                parentPhoneNumber: parentPhoneNumber || null,
  ...
  // ✅ CHANGE #1: attendance report export includes phoneNumber + parentPhoneNumber
  router.get("/admin/attendance-report/export", simpleAuth, requireRole("admin"), async (req, res) => {
    try {
      ...
      if (dateFrom) {
        ...
        if (fromDate) whereClauses.push(gte(attendanceRecords.checkInAt, fromDate));
      }
      ...
      if (centerId) whereClauses.push(eq(attendanceRecords.centerId, Number(centerId)));
      if (status) whereClauses.push(eq(attendanceRecords.status, String(status)));
      ...
      if (grade) {
        query = query.where(and(...(whereClauses.length > 0 ? whereClauses : []), eq(students.grade, String(grade))));
      }
      ...
      for (const r of rows) {
        ...
        const co = r.checkOutAt ? formatISTDateTime(r.checkOutAt) : "";
        ...
        const line = [
          ...
          escapeCsv(r.studentPhoneNumber || ""),
          escapeCsv(r.parentPhoneNumber || ""),
          escapeCsv(r.studentGrade || ""),
          escapeCsv(r.studentRoll || ""),
          escapeCsv(r.centerId ?? ""),
          escapeCsv(r.centerName || ""),
          escapeCsv(r.centerCode || ""),
          escapeCsv(r.checkInLat != null ? Number(r.checkInLat).toFixed(6) : ""),
          escapeCsv(r.checkInLng != null ? Number(r.checkInLng).toFixed(6) : ""),
          escapeCsv(co),
        ].join(",");
      ...
      res.status(500).json({ error: "admin_attendance_report_export_failed" });
    }
  });
  ...
  // attendance record patch/delete unchanged
  router.patch("/admin/attendance-records/:recordId", simpleAuth, requireRole("admin"), async (req, res) => {
    try {
      const recordId = Number(req.params.recordId);
      if (!recordId || Number.isNaN(recordId)) return res.status(400).json({ error: "invalid_record_id" });
      ...
      if (action === "set_status") {
        ...
        if (ci && checkOutAt.getTime() < ci.getTime()) return res.status(400).json({ error: "checkout_before_checkin" });
        ...
        if (ci && co && co.getTime() < ci.getTime()) return res.status(400).json({ error: "checkout_before_checkin" });
        ...
        if (!centerId || Number.isNaN(centerId)) return res.status(400).json({ error: "invalid_center_id" });

        const [center] = await db.select().from(centers).where(eq(centers.id, centerId));
        if (!center) return res.status(400).json({ error: "center_not_found" });
        ...
        patch.checkInAccuracy = req.body?.accuracy != null ? Number(req.body.accuracy) : record.checkInAccuracy;
      } else {
      ...
      await db.update(attendanceRecords).set(patch).where(eq(attendanceRecords.id, recordId));

      const [updated] = await db.select().from(attendanceRecords).where(eq(attendanceRecords.id, recordId));
  ...
  router.delete("/admin/attendance-records/:recordId", simpleAuth, requireRole("admin"), async (req, res) => {
    try {
      const recordId = Number(req.params.recordId);
      if (!recordId || Number.isNaN(recordId)) return res.status(400).json({ error: "invalid_record_id" });
